...
begin
  Free:=(C[l,k]=0) {and (check1(l,k,gen-1,1,C)or check1(l,k,gen-1,2,C))};
...
procedure TfmBattle.bbOKClick(Sender: TObject);
var i,j:integer;  puf:boolean;
begin
  memo1.Lines.Clear;
  for i:=0 to 9 do
   begin
    for j:=0 to 9 do
      Memo1.Lines.ValueFromIndex[i]:=Memo1.Lines.ValueFromIndex[i]+intToStr(A[i,j])+' ';
         ...
         Repeat
          //ShowMessage(IntToStr(state)+' state');
           ...
           Case state of
             ...
             2: begin
                  if {((y-1)>=0)and (C[y-1,z]=0)and} not (1 in S)
                     then
                      o:=1
                     else
                      begin
                      if {((z+1)<=9)and (C[y,z+1]=0) and} not (2 in S)
                         then
                          o:=2
                         else
                          begin
                          if {((y+1)<=9)and (C[y+1,z]=0) and} not (3 in S)
                             then
                              o:=3
                             else
                              begin
                              if {((z-1)>=0)and (C[y,z-1]=0) and} not (4 in S)
                                 then
                                  o:=4;
                              end;
                          end;
...
procedure TfmBattle.bbOkSetClick(Sender: TObject);
var puf:boolean;
    i,j,k,l:integer;
begin
  pnSet.Visible:=False;
...
procedure TfmBattle.cFieldMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
...
procedure TfmBattle.N2Click(Sender: TObject);
var i,j:integer;
begin
  nol:=true;
  bbOk.Visible:=False;
...
procedure TfmBattle.N9Click(Sender: TObject);
begin
  ShowMessage('Авторы: Клименков Артём группа 2-151');
end;

procedure TfmBattle.AutoClick(Sender: TObject);
var i,j,k,l:integer;
begin
  Initmas(A,A,C);
  for i:=0 to 210 do
...
procedure TfmBattle.N6Click(Sender: TObject);
begin
...
procedure TfmBattle.N3Click(Sender: TObject);
var S1,S2:string;
    i,j,k,l:integer;
begin
  nol:=false;
     ...
     then
      begin
        ...
        for i:=0 to 9 do
          begin
            j:=0;
            s1:=Copy(memo1.Lines[i],2,length(memo1.lines[i]));
           // showmessage(s1);
          // showmessage(inttoStr(pos(' ',s1)));
            while not (pos(' ',s1)=0) do
              begin
